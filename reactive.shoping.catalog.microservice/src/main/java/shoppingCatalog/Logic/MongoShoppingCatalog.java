...
public class MongoShoppingCatalog implements ShoppingCatalogService {
	...
	public Mono<Product> store(Product newProduct) {
		return Mono.just(newProduct).flatMap(boundary -> {
			if (!this.validator.isValidProduct(boundary))
				return Mono.error(() -> new ProductInvalidException("Invalid Product input"));
			else {
				return this.daoShoppingCatalog.findById(boundary.getId())// Mono<ProductEntity>
						.flatMap(entity -> {
							return Mono.error(() -> new ProductAlreadyExistException("Product already exist"));
						})// Empty
						.switchIfEmpty(Mono.defer(() -> {
							return Mono.just(boundary).map(this::toEntity)
									.flatMap(entity -> this.daoShoppingCatalog.save(entity)).map(this::toBoundary);
						}))// Mono<Object>
	...
	public Mono<Product> getProductById(String productId) {
		...
		return Mono.just(productToCheck).flatMap(boundary -> {
			if (!this.validator.isValidId(productToCheck.getId()))
				return Mono.error(
						() -> new InvalidIdException("Id must be at least 3 characters long, and at least 1 digit!"));
			else {
				return this.daoShoppingCatalog.findById(productId)
						.switchIfEmpty(
								Mono.error(() -> new ProductIsNotExistException("Product isn't exists " + productId)))// Mono<ProductEntity>
	...
	public Mono<Void> deleteAllShoppingCatalog() {
		return this.daoShoppingCatalog.deleteAll();
	}
	...
	public Flux<Product> getSortedProducts(String filterType, String value, String sortAttribute, String order) {
		...
		return Flux.just(helper).flatMap(boundary -> {
			if (!this.validator.isValidFilterType(filterType)) {
				return Flux.error(() -> new InvalidFilterTypeException(
						"Filter type must be byName/byMinPrice/byMaxPrice/byCategoryName"));
			} else {
				if (!this.validator.isValidSortAttribute(sortAttribute)) {
					return Flux.error(() -> new InvalidSortAttributeException(
							"Sort Attribute must be id/name/price/image/category"));
				} else {
				...
				if (filterType == null) {
					return this.daoShoppingCatalog
							.findAll(Sort.by(order.equals("ASC") ? Direction.ASC : Direction.DESC, sortAttribute, "id"))
							.map(this::toBoundary).log();
				}
				switch (filterType) {
				case "byName":
					if (value == null)
						return Flux.error(() -> new InvalidValueException("Please enter the filter value"));
					else
						return this.daoShoppingCatalog.findAllByName(value,
					...
					if (!this.validator.isValidPrice(value))
						...
						return this.daoShoppingCatalog
								.findAllByPriceGreaterThanEqual(Double.parseDouble(value),
										Sort.by(order.equals("ASC") ? Direction.ASC : Direction.DESC, sortAttribute,
												"id"))// Flux<ProductEntity>
